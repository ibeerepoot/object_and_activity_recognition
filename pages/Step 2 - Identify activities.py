...
def generate_activities_from_gpt(profession, object_types, api_key):
    ...
    system_prompt = """
You are an assistant specialized in semantic activity recognition. Your task is to identify high-level work activities based on a user's profession and relevant object types. Activities describe meaningful steps a user performs and often reflect actions in business processes.

### Task
1. Reflect on the user's profession and the object types involved in their work.
2. Identify a list of relevant activities that typically involve, affect, or support those object types.
3. Include both core and supportive tasks that are recognizable within the user’s domain.
...
"""
    ...
    try:
        ...
        # Strip Markdown-style JSON code block if present
...
if user_type == "A participant in our research evaluation (Academic Staff)":
    st.markdown("You are using a predefined list of activities tailored for academic staff. Please review and edit them as needed. " \
    ...
    st.markdown(
        "You can either use a predefined list of typical academic staff activities, or generate a customized list "
        "based on your profession and selected object types from Step 1. " \
        "Please review them and reflect on whether are likely to appear in your work. " \
        "You may add additional types or delete ones that are irrelevant in your work context."
    )

    with st.expander("🎓 Use predefined activities for Academic Staff"):
        if 'predefined_activities_selected' not in st.session_state:
            st.session_state['predefined_activities_selected'] = predefined_activities.copy()
    ...
    with st.expander("🧠 Generate activities using GPT-4.1"):
